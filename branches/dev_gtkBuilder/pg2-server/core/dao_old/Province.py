...
provinceTable  = Table('provincia', params["metadata"],
...
s= select([provinceTable.c.denominazione]).execute().fetchall()
if (u'Torino',) not in s or s ==[]:
    tipo = provinceTable.insert()
...
province=Table('provincia', params['metadata'],schema = params['schema'],autoload=True)
std_mapper = mapper(Province, province,properties={
...
}, order_by=province.c.denominazione
)
#filler()